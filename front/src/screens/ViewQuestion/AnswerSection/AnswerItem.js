...
const AnswerItem = (props) => {
  ...
  const onClick = async (options) => {
    const result = await confirm("Are you sure to validate the answer?", options);
    if (result) {
      validateAnswer(answer._id)
      .then((res) => {
        dispatch(
          setAlert({
        ...
        <div className="flex pl-4 pt-4">
          ...
          <div className="flex flex-col justify-between w-full text-left pl-2 mb-2">
            <div id="answer-desc" className="pb-14 whitespace-pre-line">
              ...
              />
            <div style={{ color: "green" }}> {answer.msg} </div>
            </div>
            <div className="text-right flex justify-between items-center text-xs  ">
              ...
              <button 
                className="flex items-right p-2 m-1 bg-purple-800 border-2 border-gray-200 rounded text-white hover:bg-purple-500 h-10" 
                value="click" 
                onClick={() => onClick()}
              >
                   Validate
