    ...
    <div>
      ...
      {!loading && !loadingQuestion && answers && (
        <div className="pl-2">
          ...
          {answers.map((answer, index, response) => (
            <AnswerItem key={index} response= {response} answer={answer} user={user} question_id={props.question_id}>
            <button
              onClick={answerSubmitHandler}
              className=" p-2 m-4 bg-purple-800 border-2 border-gray-200 rounded text-white hover:bg-purple-500 h-10"
            >
              Post your Answer
            </button>
            </AnswerItem>
          ...
          <textarea
            ...
            value={ans}
            type="text"
            className="whitespace-pre-line border-2 border-gray-300 rounded mr-4 mb-30 h-56 p-2 w-11/12 focus:border-purple-800 outline-none"
          />
